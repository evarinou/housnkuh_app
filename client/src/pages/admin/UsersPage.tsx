...
import { User, Search, Filter, Edit, Trash2, Plus, UserCheck, UserX, Eye, EyeOff } from 'lucide-react';
...
const UsersPage: React.FC = () => {
  ...
  const handleBulkVisibilityChange = async (visible: boolean) => {
    if (!window.confirm(`Möchten Sie die Sichtbarkeit für ${selectedUsers.length} Benutzer ändern?`)) {
      return;
    }
    ...
    try {
      ...
      setSelectedUsers([]);
      alert(`Sichtbarkeit für ${selectedUsers.length} Benutzer erfolgreich geändert`);
    } catch (err) {
      alert('Fehler beim Ändern der Sichtbarkeit');
    }
  };
        ...
        <div className="flex flex-wrap gap-4">
          ...
          {selectedUsers.length > 0 && (
            <div className="flex items-center space-x-2">
              <span className="text-sm text-gray-600">{selectedUsers.length} ausgewählt</span>
              <button
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {user.isVendor && (
                      <button 
                        ...
                        title={user.isPubliclyVisible ? 'Öffentlich sichtbar' : 'Versteckt'}
                      >
                        {user.isPubliclyVisible ? (
                          <>
